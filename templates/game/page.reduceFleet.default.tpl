   ...
   <div class="rd_planet_row">
      ...
      <div id="prow_{$ACSRow.id}" class="rd_planet_row rd_planet_row_select" onclick="planet_select({$ACSRow.id});">
         ...
         <div class="rd_planet_fleets tooltip" data-tooltip-content="
            ...
            {if $ACSRow.{$ship} != 0}
            <tr>
            <td class='reducefleet_img_ship'> <img src='./styles/theme/stellar/gebaeude/{$ship}.gif' alt='' /></td>
            <td class='reducefleet_name_ship'>                      {$LNG.tech.{$ship}}: <span class='reducefleet_count_ship'>{$ACSRow.{$ship}|number}</span></td>
            </tr>
            {/if}
            {/foreach}
            </table>"> 
            <span style="color:#0CC;">{$LNG.rd_fleet_point}</span>  {($ACSRow.sum)|number}          
         </div>
         <div class="rd_planet_status">
            ...
            {else}
            <span style="color:#0C0;">{$LNG.rd_compustion}:</span> {($ACSRow.consumption_fleet)|number}
            {/if}         
         </div>
         <div class="clear"></div>
      </div>

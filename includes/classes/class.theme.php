...
class Theme
{
	...
	function setUserTheme($Theme) {
		if(!file_exists(ROOT_PATH.'theme/'.$Theme.'/style.cfg'))
			return false;
	...
	function getTemplatePath() {
		return ROOT_PATH.'/templates/'.$this->skin.'/';
	}
	...
	function parseStyleCFG() {
		require(ROOT_PATH.'theme/'.$this->skin.'/style.cfg');
		$this->skininfo		= $Skin;
		$this->customtpls	= (array) $Skin['templates'];	
	}
	
	function setStyleSettings() {
		if(file_exists(ROOT_PATH.'theme/'.$this->skin.'/settings.cfg')) {
			require(ROOT_PATH.'theme/'.$this->skin.'/settings.cfg');
		}
	...
	static function getAvalibleSkins() {
		if(!isset(self::$Themes))
		{
			if(file_exists(ROOT_PATH.'cache/cache.themes.php'))
			{
				...
				$Skins	= array_diff(scandir(ROOT_PATH.'assets/theme/'), array('..', '.', '.svn', '.htaccess', 'index.htm'));
				$Themes	= array();
				foreach($Skins as $Theme) {
					if(!file_exists(ROOT_PATH.'theme/'.$Theme.'/style.cfg'))
						continue;
						
					require(ROOT_PATH.'theme/'.$Theme.'/style.cfg');
